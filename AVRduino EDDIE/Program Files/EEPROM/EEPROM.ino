/*
      ...
      Simple routines to read and write to the on board eeprom..
...
/* Declare an instance of EEprom and set the address */
EEprom eprom(0x52);
...
int main() {
  ...
  */
  ...
  for (uint8_t x = 0; x < 127; x ++) {
    Serial.print(x);
    Serial.print("\t");
    Serial.println(eprom.read(x));
  }

  /* Write some integers */
  for (uint16_t x = 300; x < 402; x += 2)
  {
    eprom.write_int(x, 1000 + x);
  }
  ...
  for (uint16_t x = 300; x < 402; x += 2) {
    Serial.print(x);
    Serial.print("\t");
    Serial.println(eprom.read_int(x));
  }

  /* Useful with Tera Term to clear the serial */
  Serial.flush();
